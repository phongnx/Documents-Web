...
| `/share/d/:id` · `/share/f/:id` | `SharePage` / `SharedFolderPage` | Xem công khai, chỉ đọc (nằm **ngoài** lớp đăng nhập) + nút **⬇ Tải về** tài liệu đang xem. |
...
| `/board/kpi` | `BoardKpiListPage` | Quản lý KPI member: tạo page log + link share, gán project, khóa/đổi link, quy chế chấm điểm, 🔄 đồng bộ snapshot cho mọi member. |
...
- **Export**: 2 nút **upload thẳng vào quản lý tài liệu** (type `html`, xem bằng iframe; không tải file — cần file `.html` thì download bên trang documents) — bản chi tiết vào folder `THSOFT - Weekly Plan` (tên `mobile_team_weekly_plan_MM-DD_to_MM-DD-YYYY`), bản release/test vào sub-folder `Tester` (`plan_team_mobile_release_test_…`); folder tự tạo nếu chưa có, re-export cùng tuần hỏi thay thế (ghi đè giữ id → share link cũ vẫn sống). **Nút Export bị khóa tới khi `DocumentsContext.loading` tắt** — export lúc `folders`/`documents` còn rỗng thì `ensureFolder` tạo folder TRÙNG TÊN và `commitItems` tạo file mới thay vì hỏi ghi đè, bản export mới nằm lạc sang folder khác (đã xảy ra 08–09/09/2026). Trùng tên vẫn còn thì luôn lấy folder có `createdAt` sớm nhất. `loading` chờ CẢ documents lẫn folders; `addFolder` chỉ hạ sub-folder về gốc khi parent THẬT SỰ là sub-folder — không hạ chỉ vì parent vừa tạo chưa kịp vào state (lỗi này từng đẻ ra folder `Tester` mồ côi ở gốc).
...
- **Đồng bộ snapshot:** `meta` của sheet member (quy chế, project được gán, mốc release, bảng estimate) chỉ được ghi lại khi có mutation chạm task/quy chế hoặc khi leader mở trang chấm của member đó. Đổi cách dựng snapshot (VD nâng cap số dòng mô tả) → bấm **🔄 Đồng bộ tất cả** ở `/board/kpi` để đẩy lại cho MỌI member trong 1 lần ghi nguyên tử (`syncAllKpiSheetMeta`), khỏi phải mở lần lượt từng trang.
...
- **Trang member** (`/share/kpi/:token`): log theo Start/End time (duration tự tính, **trừ giờ nghỉ trưa 11:45–13:15**), nhóm theo ngày + dòng Tổng; **validate field tối thiểu** khi lưu (start/end hợp lệ + giai đoạn + project + task + **mốc release**); **rule nghỉ phép** (nghỉ cả ngày không log được, nửa ngày không đè giờ nghỉ, mốc 12:00); ⧉ nhân bản dòng; **auto-lưu dòng đang sửa dở** khi bấm ＋/⧉/click dòng khác (thiếu field → báo lỗi và chặn action); **điểm tự chấm** mặc định 0 (sửa được tới khi leader chấm; tổng dùng điểm leader nếu có); dòng đã chấm bị khóa; ℹ️ xem quy chế (chỉ đọc). KPI tháng = 100 + Σ điểm. **Gắn mốc release/milestone:** dưới ô Project của dòng đang sửa có select **🚀 Mốc** — **BẮT BUỘC chọn** (nguồn: snapshot releases + bảng estimate tham gia + mốc đã dùng trong tháng + "Khác…" nhập tay; pick task từ estimate tự gắn mốc theo bảng). Việc không thuộc release nào (họp, nghiên cứu chung, hỗ trợ…) chọn mục cố định **"— Không thuộc mốc —"** (hằng `NO_MILESTONE`) — chọn có ý thức thay vì bỏ trống, bảng tổng kết gom thành nhóm riêng có tên rõ ràng. Dòng cũ chưa có mốc chỉ bị bắt điền khi member mở ra sửa; rules KHÔNG siết `rel` (chỉ validate ở UI, tránh chặn cứng đường ghi ẩn danh); lưu denormalized trên entry (`rel: {label, date?}`), dòng có mốc hiện 🚀 cạnh project, bảng Tổng kết tháng gom nhãn mốc của đoạn hiện dưới ô App. **Plan trước:** nút "＋ Plan hôm sau" thêm dòng cho ngày làm việc kế tiếp (T6/7/CN → T2; ngày tương lai không bắt giờ, block ngày có badge 📋 Plan — đến ngày thì sửa dòng điền giờ thực tế); cuối tuần (từ T6) có nút "📋 Plan tuần sau" lưu plan chung cả tuần (`weekPlans/{thứ-2}`, text tự do) hiện thành block trên bảng cho cả member lẫn leader. Header hiện **🚀 Release sắp tới** của các app được gán (snapshot `meta.releases` từ task của leader, tự refresh ở MỌI mutation chạm task: thêm/sửa/xóa/gán app/done từ plan tuần + khi gán project/leader mở trang chấm — helper `kpiReleaseWrites`): tối đa 2 mốc gần nhất, mốc trong tuần highlight, nhiều hơn thì thu gọn + nút expand; click chip → dialog chi tiết các task cùng version của release đó (badge trạng thái + mô tả; cap **15 task/mốc, 40 dòng mô tả/task** — chạm trần mới hiện dòng `… còn N dòng nữa`, member không có cách nào xem tiếp vì họ chỉ đọc snapshot này).
...
- **Tự tổng hợp** (`buildMemberRows`/`aggregateKpiSummaryMembers` trong `src/lib/kpiSummary.ts`): mỗi member có log/nghỉ phép trong tháng → các đầu mục **phân vùng theo dòng thời gian, không chồng lấn** (mỗi ngày thuộc đúng 1 đoạn): loại việc **chủ đạo** của ngày = project + category có tổng phút log lớn nhất (hòa → nhiều dòng hơn → nối mạch ngày trước); ngày liên tiếp cùng chủ đạo gộp 1 đoạn (ngày trống không cắt); **các đoạn cùng `project + giai đoạn + mốc release chủ đạo` gộp thành 1 đầu mục** kể cả không liền kề (4 đợt fix bugs rải rác cho cùng mốc = 1 dòng; mốc khác nhau v1.116 vs v1.117 vẫn 2 dòng) — mốc chủ đạo = nhãn `rel` nhiều nhất trong các dòng CHỦ ĐẠO của đoạn, cột **Ngày** hiện tổng ngày công của RIÊNG các đợt kèm dòng nhỏ liệt kê `12/08–14/08 · 18/08 · 25/08–28/08` (`spans`, nối qua T7/CN, ngày trống giữa tuần thì cắt); item lẻ trong đoạn gom theo NHÃN `(project · loại)` — mỗi nhãn 1 dòng, tối đa 5 nhãn (fix bugs đếm ticket, loại khác `- (Nhãn) — N việc: A; B`), KPI + lý do ± của đoạn tính trên mọi dòng log. Cột Tasks là bản **tổng hợp ngắn gọn** (không liệt kê chi tiết):
  - Nhóm **fix bugs**: mọi feature dạng ticket gom đúng 1 dòng `- Xử lý ticket — N ticket` (member hay ghi số ticket vào cột **Feature** — nhận cả typo "Ticker" lẫn dạng chỉ có `#123`; `Ticket #116, #117, #112` = 3 ticket, đếm theo số lần `#` để không nhân đôi khi Task cũng ghi ticket); feature thật có ≥2 việc thì mỗi feature 1 dòng `- {feature} — N ticket + M việc`; các feature chỉ có ĐÚNG 1 việc gom chung 1 dòng liệt kê tên `- A, B, C — 3 việc`.
  - Nhóm khác: trong mỗi feature, các việc **cùng tiền tố** gom thành 1 mục `- lock firebase event — 7 việc: show_ad_ev; consent_ev; …` (member hay log kiểu `lock firebase event: <tên event>` mỗi event 1 dòng) — **hai tầng**: (1) theo phần trước dấu `:` đầu tiên (≤60 ký tự) — bắt được `FIX-CORE:` ×15; (2) các mục còn lại gom theo **tiền tố chung của từ** (chung ≥3 từ VÀ ≥12 ký tự, nhãn = phần chung dài nhất) — bắt được `update logic realtime notifications - §3: …` mà tầng 1 bỏ sót vì dấu `:` rơi vào giữa đuôi. Ngưỡng 2 từ bị loại vì gom nhầm (`update logic` nuốt cả việc không liên quan). Cả hai tầng đều cần ≥2 dòng; tiền tố chỉ có 1 dòng thì giữ nguyên văn, đuôi rỗng chỉ tính số không in. Sau khi gom: còn đúng 1 mục → 1 dòng `- {feature}: việc` (feature trùng nội dung việc thì chỉ in việc); nhiều mục → `# feature (N việc):` + tối đa 3 mục đầu (cắt ≤90 ký tự) + `- … và N việc khác` (**N đếm theo việc gốc**, không theo mục). Dấu gạch/bullet member tự gõ ở đầu ô Task được strip (không thì bảng in ra `- - việc`).
  - Mỗi đoạn liệt kê tối đa **8 nhóm feature** (ưu tiên nhóm nhiều việc nhất, in theo thứ tự xuất hiện), phần dư gom `- … và N việc ở M mục khác: A, B, C`; feature/việc trùng nhau khác hoa–thường được gộp chung. Đo trên log tháng 08/2026 (1.014 dòng): 517 → 253 dòng trong cột Tasks, 73 → 50 đầu mục, ô dài nhất 56 → 26 dòng, tổng KPI từng member không đổi, KPI/ngày công/mốc release không đổi. Start/End = min/max ngày log, Ngày = số ngày làm việc T2–T6, KPI nhóm = Σ điểm ± các dòng log (leader chấm ?? tự chấm) kèm **lý do cộng/trừ** (`kpiNote` — mỗi dòng log điểm ≠ 0 một dòng: ưu tiên reason leader ghi khi chấm, không có thì lấy text task; hiện dưới badge ± ở cả trang share, sửa tay được), **KPI tháng = 100 + Σ**; cột **Ngày nghỉ** = nghỉ phép từ `leaves` (đợt vắt tháng chỉ tính phần trong tháng) + ô **Nghỉ lễ chung** điền tay ở header (áp mọi member).
...
- **🧹 Tối ưu bảng** (`optimizeSummaryRows`/`optimizeSummaryMembers`): gộp các dòng ĐANG CÓ TRÊN BẢNG cùng `App + Giai đoạn + Mốc` — dùng khi tổng hợp xong mới điền bổ sung mốc release cho các dòng còn thiếu, khỏi phải tổng hợp lại (tổng hợp lại sẽ đè hết sửa tay). Nguồn là các dòng trong bảng, KHÔNG đọc lại log: Start/End = sớm nhất/muộn nhất, ⏱ các đợt = union khoảng của các dòng (đọc cột ⏱ nếu có, không thì Start–End; hỏng định dạng thì fallback Start–End), Ngày = tổng ngày công của các đợt sau union (không cộng dồn, tránh đếm trùng), Tasks/Tiến độ/Lý do ± nối lại **bỏ dòng trùng**, KPI cộng số (có ô gõ tay không phải số thì nối bằng ` · ` để không mất chữ). Dòng trống cả App lẫn Giai đoạn (ghi chú tự thêm) không bị gộp; chạy 2 lần không đổi gì; chỉ đổi trên form, bấm 💾 Lưu mới ghi.
- **Phân tách member**: mỗi member là 1 `<tbody>` — kẻ đậm giữa các khối + nền xen kẽ; ô Tên hiện luôn **KPI tháng** ngay dưới tên (cột KPI tháng nằm cuối bảng, cách tên cả nghìn pixel khi cuộn ngang). Điểm KPI tháng tô màu theo `kpiScoreClass`: >100 xanh lá, =100 xanh dương, <100 đỏ.
