...
| `/board/kpi` | `BoardKpiListPage` | **KPI member log**: danh sách member (KPI tháng = 100 + Σ điểm, tổng giờ), tạo member + **copy link share**, 🧩 **gán list project** (member chỉ chọn được project được gán khi log; bỏ gán hết = nhập tự do), 🔁 đổi link, 🔒 khóa, xóa; nút ⚖️ quản lý **quy chế chấm điểm** (5 giai đoạn + mức điểm, sửa được). |
| `/board/kpi/:memberId` | `BoardKpiMemberPage` | Xem log của 1 member + **chấm điểm từng dòng** (popover với nút chấm nhanh theo quy chế, delta/lý do tự do), xóa dòng kèm điểm. Leader không sửa hộ nội dung — muốn sửa thì mở link share. |
| `/share/kpi/:token` | `KpiSharePage` | **Công khai theo link riêng (capability URL)** — member tự log task hàng ngày (Start/End time → duration tự tính, giai đoạn/project/feature/task/note, nhóm theo ngày + dòng Tổng), **validate field tối thiểu khi lưu dòng** (start/end hợp lệ + giai đoạn + project + task — thiếu thì viền đỏ ô + báo lỗi, không cho lưu), nút **⧉ nhân bản** dòng thành dòng mới (copy nội dung, giờ seed nối ca, điểm về 0), khi lưu dòng có ô **điểm TỰ CHẤM mặc định 0** (`entry.selfDelta`, badge viền đứt, member sửa được tới khi leader chấm; tổng ngày/tháng dùng điểm leader nếu có, fallback tự chấm), **dòng đã leader chấm bị khóa sửa toàn bộ** (rule chặn cả server); nút **ℹ️ Quy chế** xem preview quy chế chấm điểm (chỉ đọc, snapshot trong `meta.rules`). |
