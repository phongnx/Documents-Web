...
| `/board/report` | `BoardReportListPage` | Danh sách **báo cáo ngày**; báo cáo mới nhất mở sẵn, cũ hơn collapse; tạo báo cáo hôm nay hoặc **clone từ báo cáo gần nhất**; nút **📋 Copy** text từng báo cáo ngay tại card (clipboard bị chặn → fallback tải `.txt`). |
...
**Quản lý danh mục (nút ⚙️ Danh mục trên thanh nav Bảng dự án):** loại task (`meta.taskTypes`) thêm/sửa (đổi tên cascade mọi task)/xóa (chặn nếu còn task dùng); **loại nhánh plan** (`meta.planCategories`) thêm/sửa/xóa (không sửa/xóa preset release/test/ads/plan/other; đổi tên cascade category trong mọi plan) — plan editor đọc dropdown "loại nhánh" từ danh mục này; loại milestone (`meta.milestoneTypes` — mỗi loại `{key,label,isRelease}`) thêm/sửa label+cờ isRelease/xóa (không xóa 2 loại gốc `release`/`test`). Cờ `isRelease` quyết định nhánh có milestone đó có được tính là "release" ở card tiến độ và Lịch release (`releaseKeysOf`). Sync từ báo cáo dùng luật **strict**: chỉ đánh ✅ Xong khi có tín hiệu rõ (hoàn thành / đã release / 100% / ✅) và không có "tester/đang check/đang fix"; có build/release nhưng chưa done → 🧪 Test. **Nhánh có milestone**: tín hiệu Xong chỉ được lấy từ dòng nói về **chính mốc đó** (chứa `build/release/submit/publish/store/phát hành` hoặc khớp `milestone.text`) — một task lẻ hoàn thành trong project **không** làm mốc release thành Xong (tránh mark nhầm 100% khi báo cáo chưa đề cập build release). Ngoài ra, nếu nội dung nhánh chứa cụm **"fix bug(s) phát sinh"** hoặc **"build test"** → ép về 🧪 Test (ưu tiên hơn tín hiệu Xong, vì hàm ý chưa hoàn tất).
