...
| `/share/kpi/:token` | `KpiSharePage` | **Công khai theo link riêng (capability URL)** — member tự log task hàng ngày (Start/End time → duration tự tính, giai đoạn/project/feature/task/note, nhóm theo ngày + dòng Tổng), thấy điểm đã chấm nhưng **dòng đã chấm bị khóa sửa** (rule chặn cả server). |
