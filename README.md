...
| `/board/plan` | `BoardPlanListPage` | Danh sách plan tuần; nút **Tạo plan tuần (tự động)** sinh nội dung từ dữ liệu thật (`src/lib/planAutofill.ts`): (1) carry-over nhánh **chưa done** của plan tuần trước (giữ state, bỏ % cũ), (2) fill task **đang chạy** theo app (bỏ task chưa gán app/app archived, không trùng nhánh carry-over), (3) đề xuất nhánh **release** cho task có `planDate` trong tuần + tự fill timeline theo thứ; không có gì để fill → fallback template mẫu. Nút **📋 Release** trên mỗi card: copy các app đã đạt mốc release trong tuần (nhánh release `done`) **phân mục đánh số** — tiêu đề ưu tiên text mục Timeline (app + phiên bản, match theo token không phụ thuộc thứ tự từ) + **ngày done** suy từ báo cáo ngày (ngày đầu tiên báo done, cùng bộ luật sync; done tay → "không rõ ngày"), content là items của nhánh; sắp theo thứ tự Timeline, không match → xếp sau theo ngày done. |
